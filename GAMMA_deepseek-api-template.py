...
def generate_response(client, prompt_text, prompt_id, rate_limiter):
...
def process_single_prompt(client, prompt_data, rate_limiter, results_manager, df):
    ...
    Args:
        ...
        prompt_data: Dictionary containing prompt_id and prompt_text
    ...
    result_content, reasoning_content, response = generate_response(
        client, prompt_text, prompt_id, rate_limiter
    )
...
def process_prompts(client, path_to_prompts):
    ...
    # Prepare list of all prompts
    all_prompts = [
        {"prompt_id": row[PROMPT_INDEX_COLUMN], "prompt_text": row[PROMPT_COLUMN]}
